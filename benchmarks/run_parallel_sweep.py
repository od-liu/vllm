...
"""
Parallel Configuration Sweep Script for Operator Benchmarking.
...
def merge_phase_results(phase1_file: str, phase2_file: str, output_file: str):
    ...
    try:
        ...
        merged = {
            ...
            "e2e_metrics": phase1_data.get("e2e_metrics"),
    ...
    try:
        ...
        if e2e is not None:
            ...
            if 'throughput' in e2e:
                metrics['throughput_tokens_per_sec'] = e2e['throughput'].get('tokens_per_second', None)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run parallel configuration sweep for operator benchmarking"
    )
    parser.add_argument(
        "--num-gpus",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Include configurations with DP > 1 (Note: vLLM's LLM class does not support this in single-process mode)"
    )
    ...
    print("=" * 80)
    print(f"Parallel Configuration Sweep")
    print("=" * 80)
    ...
    print(f"Constraint: TP >= DP (ensures valid configurations)")
    print(f"  Note: All TP/DP combinations where TP >= DP will be tested")
    print(f"Number of configurations: {len(configs)}")
    print("\nConfigurations to run:")
    dp_warning_shown = False
    for tp, pp, dp in configs:
        status = ""
        if dp > 1:
            status = " ⚠️  (Will fail: DP > 1 requires multi-process setup)"
            if not dp_warning_shown:
                print(f"  ⚠️  Note: Configurations with DP > 1 will fail because")
                print(f"     vLLM's LLM class does not support data_parallel_size > 1")
                print(f"     in single-process mode. See TP_VS_DP_EXPLANATION.md for details.")
                dp_warning_shown = True
        print(f"  TP={tp}, PP={pp}, DP={dp}{status}")
    print("=" * 80)
